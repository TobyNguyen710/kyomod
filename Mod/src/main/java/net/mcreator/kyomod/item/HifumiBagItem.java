...
import net.mcreator.kyomod.item.inventory.HifumiBagInventoryCapability;
...
public class HifumiBagItem extends Item {
	public HifumiBagItem() {
		super(new Item.Properties().tab(KyomodModTabs.TAB_BLUE_ARCHIVE).stacksTo(1).rarity(Rarity.UNCOMMON));
	}
		...
		if (entity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openScreen(serverPlayer, new MenuProvider() {
				...
				public Component getDisplayName() {
					return Component.literal("Hifumi Bag");
				}
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					return new HifumiBagGUIMenu(id, inventory, packetBuffer);
				}
			}, buf -> {
	...
	public ICapabilityProvider initCapabilities(ItemStack stack, @Nullable CompoundTag compound) {
		return new HifumiBagInventoryCapability();
	}
